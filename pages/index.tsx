            ...
            listings.map((listing) => (
              <div className="bg-white shadow-lg border rounded-lg overflow-hidden" key={listing.vin}>
                ...
                <div className="p-4">
                  ...
                  <div className="text-gray-600 font-bold">{listing.score?.toFixed(1)}</div>
                  <div className="text-gray-600 pl-3">{listing.weightedRating?.toFixed(1)}:</div>
                  ...
                  <div className="text-gray-600 pl-3" title={`price per remaining mile, expecting ${milesExpected} miles`}>
                    {listing.pricePerRemainingMiles ? `${listing.pprmScore?.toFixed(1)}: $${listing.pricePerRemainingMiles.toFixed(2)} / rem mi` : ''}
                  </div>
                  <div className="text-gray-600 font-bold pl-6">${listing.price_approx?.toLocaleString() ?? 'N/A'}</div>
                  <div className="text-gray-600 pl-6">{listing.mileage?.toLocaleString() ?? 'N/A'} miles</div>
